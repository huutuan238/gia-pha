    ...
    <div v-else-if="event" class="card framed event-detail">
      ...
      <div class="event-detail-grid">
        <div class="detail-item">
          ...
          <span class="detail-sub" v-if="event.lunerDateTime"
            >{{ event.lunerDateTime }} (Âm lịch)</span
          >
        </div>
...
export default {
  ...
  computed: {
    personName() {
      ...
      const person = this.familyMembers.find(
        (p) => p.id === this.event.personId,
      );
      if (!person) return "";
    ...
    async onDelete() {
      ...
      try {
        ...
        this.deleteError =
          ...
          "Không thể xoá sự kiện. Vui lòng thử lại.";
        this.deleting = false;
      }
    },
