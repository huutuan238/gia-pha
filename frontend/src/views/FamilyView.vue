  ...
  <main id="main" class="section container" style="max-width: 960px">
    ...
    <div class="family-grid" v-if="!loading && families.length">
      ...
      >
        ...
        <p class="family-address" v-if="family.ancestralHouseAddress">
          {{ family.ancestralHouseAddress }}
        </p>
        <p class="family-desc" v-if="family.description">
          {{ family.description }}
        </p>
        <div class="family-actions">
          <button
            ...
            "
            @click="openEditPanel(family)"
          >
            Sửa
          </button>
          <button
            class="btn btn-outline"
            style="color: var(--color-seal); border-color: var(--color-seal)"
            @click="askDelete(family)"
          >
            Xoá
          </button>
        </div>
  ...
  <Teleport to="body">
    <div v-if="panel.open" class="modal-overlay" @click.self="closePanel">
      <div class="paper modal-card">
        <button class="modal-close" @click="closePanel" aria-label="Đóng">
          ✕
        </button>
        ...
        <form @submit.prevent="submitPanel">
          <div class="form-grid">
            ...
            <div class="field full">
              ...
              <input
                v-model="form.ancestralHouseAddress"
                type="text"
                placeholder="Làng Đông Bàn, Hải Dương"
              />
            </div>
            <div class="field">
              <label
                >Vĩ độ
                <span class="hint">(tuỳ chọn, cho Google Map)</span></label
              >
              <input
            ...
            <div class="field full">
              ...
              <textarea
                ...
                rows="4"
                placeholder="Vài dòng giới thiệu nguồn gốc, truyền thống…"
              ></textarea>
            </div>
          </div>

          <div class="form-actions">
            <button type="button" class="btn btn-outline" @click="closePanel">
              Hủy
            </button>
            <button
              type="submit"
