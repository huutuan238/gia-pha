<template>
    <main id="main" class="section container">
      <button class="btn btn-ghost btn-sm back-btn" @click="goBack">← Quay lại danh sách album</button>
  
      <div v-if="loading" class="tree-status">Đang tải album...</div>
  
      <div v-else-if="loadError" class="tree-status tree-status-error">
        {{ loadError }}
        <button class="btn btn-outline" style="margin-left:12px;" @click="getAlbumDetail">Thử lại</button>
      </div>
      ...
      <template v-else-if="album">
        <div class="tree-toolbar">
          <div>
            ...
            <p v-if="album.description" style="margin-top: 4px">{{ album.description }}</p>
          </div>
          <button class="delete-btn" @click="onDeleteAlbum" :disabled="deletingAlbum">
        ...
        >
          ...
          <p v-if="uploading" style="margin-top:8px">Đang upload {{ uploadingCount }} ảnh...</p>
        </div>
        <p v-if="uploadError" class="modal-error">{{ uploadError }}</p>
  
        <!-- LƯỚI ẢNH -->
        <div class="photo-grid" style="margin-top: 24px">
          <p v-if="album.photos.length === 0" class="tree-status">Album chưa có ảnh nào.</p>
  
          <div class="photo-tile" v-for="photo in album.photos" :key="photo.id">
            <img :src="resolvePhotoUrl(photo.url)" :alt="photo.caption || album.title" class="photo-img">
            <div class="photo-overlay">
              <span v-if="photo.caption" class="photo-caption">{{ photo.caption }}</span>
              <button class="photo-delete-btn" @click="onDeletePhoto(photo.id)" title="Xoá ảnh">✕</button>
            </div>
          </div>
        </div>
      </template>
  ...
  export default {
    ...
    methods: {
      ...
      async onDeleteAlbum() {
        if (!confirm(`Xoá album "${this.album.title}" và toàn bộ ảnh bên trong?`)) return;
