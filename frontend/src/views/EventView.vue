<template>
    <main id="main" class="section container" style="max-width: 840px">
      ...
      <div v-else-if="loadError" class="tree-status tree-status-error">
        {{ loadError }}
        <button class="btn btn-outline" style="margin-left:12px;" @click="getEventInfos">Thử lại</button>
      </div>
      ...
      <table v-else class="event-table paper">
        ...
        <tbody>
          ...
          >
            ...
            <td>{{ event.type }}</td>
            <td class="cell-name">{{ event.personName || getPersonName(event.personId) }}</td>
    ...
    methods: {
      async getEventInfos() {
        ...
        try {
          ...
          this.eventInfos = Array.isArray(payload) ? payload : (payload.items || payload.data || []);
        } catch (error) {
