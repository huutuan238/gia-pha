<template>
    ...
    <template v-else>
      ...
      <section v-show="activeTab === 'families'">
        <div class="tree-toolbar" style="margin-top: 24px">
          ...
          <button class="btn btn-primary" @click="openFamilyPanel()">
            + Thêm dòng họ
          </button>
        </div>
        ...
        <div class="family-grid">
          <div
            class="paper family-card"
            v-for="family in families"
            :key="family.id"
          >
            <h3>{{ family.name }}</h3>
            <p class="family-meta" v-if="family.foundedYear">
              Lập họ năm {{ family.foundedYear }}
            </p>
            <p class="family-meta" v-if="family.branchNumber">
              {{ family.branchNumber }} chi nhánh
            </p>
          ...
          <p v-if="!families.length" style="color: var(--color-cream-dim)">
            Chưa có dòng họ nào.
          </p>
        </div>
      </section>
      ...
      <section v-show="activeTab === 'filter'">
        <p
          ...
          "
        >
          Dùng để lấy danh sách người thoả điều kiện, phục vụ tính đóng góp hoặc
          lập danh sách tổ chức sự kiện. Chỉ lọc trong số người
          <strong>còn sống</strong>.
        </p>

        <p v-if="personsLoadError" class="alert-error">
          {{ personsLoadError }}
        </p>
        <p v-if="usingMockPersonsData" class="alert-info">
          ⚠️ Đang dùng dữ liệu mẫu (demo) — backend GET /api/persons/ chưa sẵn
          sàng hoặc chưa kết nối.
        </p>
        <p v-if="personsLoading" style="color: var(--color-cream-dim)">
          Đang tải dữ liệu…
        </p>
          ...
          <div v-if="hasSearched" style="margin-top: 28px">
            <div class="result-header">
              <p class="result-count">
                Tìm thấy <strong>{{ results.length }}</strong> người thoả điều
                kiện.
              </p>
              <button
                type="button"
              ...
              <table class="user-table">
                ...
                <tbody>
                  <tr v-for="p in paginatedResults" :key="p.id">
                    ...
                    <td>{{ p.birthYear || "—" }}</td>
                    <td>
                      {{ p.birthYear ? CURRENT_YEAR - p.birthYear : "—" }}
                    </td>
              ...
              <div v-if="results.length" class="pagination-bar">
                <span class="pagination-info">
                  Trang {{ currentPage }} / {{ totalPersonPages }} ({{
                    (currentPage - 1) * PERSON_PAGE_SIZE + 1
                  }}–{{
                    Math.min(currentPage * PERSON_PAGE_SIZE, results.length)
                  }}
  ...
  <Teleport to="body">
    <div
      v-if="familyPanel.open"
      class="modal-overlay"
      @click.self="closeFamilyPanel"
    >
...
async function submitFamily() {
  ...
  const payload = {
    ...
    founded_year:
      familyForm.foundedYear === "" ? null : Number(familyForm.foundedYear),
    branch_number:
      familyForm.branchNumber === "" ? null : Number(familyForm.branchNumber),
