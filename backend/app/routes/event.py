...
def update_event(event_id):
    ...
    if event is None:
        return jsonify({"message": "Event not found"}), 404
    event.family_id = body.get("family_id", event.family_id)
    if body.get("datetime"):
        event.event_datetime = datetime.fromisoformat(body["datetime"])
    event.event_type = body.get("type", event.event_type)
    ...
    event.notified = body.get("notified", event.notified)
    event.recipient_count = body.get("recipients", event.recipient_count)
    db.session.commit()
